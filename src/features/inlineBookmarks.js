...
class Commands {
    ...
    showSelectBookmark(filter, placeHolder) {
        ...
        vscode.window.showQuickPick(entries, { placeHolder: placeHolder || 'Select bookmarks' }).then(item => {
            vscode.commands.executeCommand("rebol.jumpToRange", item.target.uri, item.target.range);
        });
    }
    ...
    showListBookmarks(filter) {
        ...
        if (!vscode.window.outputChannel) {
            vscode.window.outputChannel = vscode.window.createOutputChannel('rebol');
        }
...
class InlineBookmarkTreeDataProvider {
    ...
    constructor(rebolController) {
        this._onDidChangeTreeData = new vscode.EventEmitter();
        this.onDidChangeTreeData = this._onDidChangeTreeData.event;

        this.controller = rebolController;
        this.model = new rebolDataModel(rebolController);
