...
const {rebolCtrl, InlineBookmarkTreeDataProvider} = require('./features/inlineBookmarks');
...
function onActivate(context) {
    const auditTags = new rebolCtrl(context);
    const treeDataProvider = new InlineBookmarkTreeDataProvider(auditTags);

    var activeEditor = vscode.window.activeTextEditor;

    /** register views */
    const treeView = vscode.window.createTreeView('rebolExplorer', { treeDataProvider: treeDataProvider });
    /*
    context.subscriptions.push(treeView);
    */
    /*
    context.subscriptions.push(
        vscode.window.registerTreeDataProvider("rebolExplorer", treeDataProvider)
    );
    */
    
    /** register commands */
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.jumpToRange", (documentUri, range) => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.refresh", () => {
            auditTags.commands.refresh();
            treeDataProvider.refresh();
        })
    );
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.toggleShowVisibleFilesOnly", () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.toggleViewKeepFilesExpanded", () => {
            settings.extensionConfig().update("view.expanded", !settings.extensionConfig().view.expanded);
        })
    );
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.jumpToNext", () => {
            let element;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.jumpToPrevious", () => {
            let element;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.setTreeViewFilterWords", (words) => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.debug.state.reset", () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.showSelectBookmark", () => {
            auditTags.commands.showSelectBookmark();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.showSelectVisibleBookmark", () => {
            auditTags.commands.showSelectVisibleBookmark();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.listBookmarks", () => {
            auditTags.commands.showListBookmarks();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.listVisibleBookmarks", () => {
            auditTags.commands.showListVisibleBookmarks();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand("rebol.scanWorkspace", () => {
